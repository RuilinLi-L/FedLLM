...
> 先用 `train.py` 训练并保存一个真实存在的 LoRA `.pt` 文件，再把这个 `.pt` 传给 `peft_eval.sh` 或 `peft_baselines.sh`。
...
```text
./models/gpt2_sst2_lora_r16/final.pt
./models/gpt2_sst2_lora_r16/final_tokenizer/
```

后续 PEFT/LoRA eval 应优先使用这个 `final.pt`：

```bash
LORA_CKPT=./models/gpt2_sst2_lora_r16/final.pt
```

注意：

- `--lora_r` 必须和训练 checkpoint 的 rank 一致；上面训练是 `16`，评测也要传 `--lora_r 16`。
- `--model_path` 必须和训练 checkpoint 的 backbone 一致；上面训练是 `gpt2`，评测也要传 `gpt2`。
- 当前 eval 只支持本地 `.pt/.pth` `state_dict` 文件，不能直接传 PEFT 原生 adapter 目录。
- 中途 `--save_every` 保存的 checkpoint 目录当前不适合作为 eval 输入；最稳的是使用最终的 `final.pt`。

### 5.3 快速检查 checkpoint

正式跑实验前，可以先确认文件存在且能被 PyTorch 读取：

```bash
test -f "$LORA_CKPT" && ls -lh "$LORA_CKPT"

python - <<'PY'
import torch
path = "./models/gpt2_sst2_lora_r16/final.pt"
state = torch.load(path, map_location="cpu")
print(path)
print("num_tensors =", len(state))
print("first_key =", next(iter(state)))
PY
```
...
- 正常运行 LoRA 下支持的 `none / noise / topk / compression / lrb`
- 将 LoRA 下不支持的 `dpsgd / soteria / mixup / dager` 记录为 `unsupported`
...
Llama 路线同样要求传入真实存在的 `.pt/.pth` LoRA checkpoint。示例：

```bash
LLAMA_LORA_CKPT=./models/llama_sst2_lora_r256/final.pt
