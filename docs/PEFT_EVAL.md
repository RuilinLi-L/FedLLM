...
- PEFT eval：`lora / ia3 / prefix`
- PEFT training：训练期 post-gradient `none / noise / topk / compression / lrb / lrbprojonly`
...
- 训练期完整 DP-SGD / Soteria / MixUp / DAGER direct-generation defense。它们仍作为 eval baseline，不作为训练期 PEFT defense
...
`--peft_method` 省略时默认是 `lora`。PEFT adapter 目录会读取 `adapter_config.json`，并校验：
...
- Projection-LRB 是主方法
- IA3/Prefix/BERT 是泛化实验和机制补强
- representation-side bottleneck 是 forward-side ablation，不声明 formal DP
- `dpsgd / soteria / mixup` 是 attack-time eval baseline，不代表完整训练期实现
