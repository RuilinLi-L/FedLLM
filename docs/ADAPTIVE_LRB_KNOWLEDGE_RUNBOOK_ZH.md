...
- `oracle` 与旧 defense-aware DAGER 等价；新知识模型均为 opt-in。
...
冒烟必须满足：所有目标日志只有一个完整 summary、`result_status=ok`、`n_inputs_completed=2`，且 `method_only` 的审计字段为 `ratio_knowledge=hidden`、`sign_knowledge=hidden`。
...
以下命令不依赖新知识模型，可用于补齐同预算 white-box baseline：
