...
- `--metrics` controls the primary reported metric fields for `mse`, `psnr`, `ssim`, and `patch_recovery`; LPIPS remains `n/a` until a runtime dependency is wired in.
...
| `Clustering.py` | `cluster_and_reassemble` plus official tag grouping | Deterministic cluster fallback and tag-based direct grouping | Official clustering algorithm parity |
...
- `ssim` and `lpips`; `lpips` is emitted as `n/a` unless a real LPIPS dependency is wired into the runtime.
