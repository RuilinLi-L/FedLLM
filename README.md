...
Running any helper script under `scripts/` (for example `./scripts/gpt2.sh ...`) **automatically** saves a full terminal transcript under `log/runs/` via `tee`. The script prints `[dager] Terminal log: ...` to the real terminal before output is duplicated.

- Change the directory: `DAGER_LOG_DIR=/path/to/dir ./scripts/gpt2.sh sst2 4`
- Turn off file logging: `DAGER_NO_AUTO_LOG=1 ./scripts/gpt2.sh sst2 4`

`python3 train.py ...` (see Fine-tuning below) also creates `log/runs/train_<dataset>_<timestamp>.txt` by default, with the same environment variables. Use `--log_file PATH` to set an explicit path, or `DAGER_NO_AUTO_LOG=1` to skip logging.
...
git clone https://github.com/jeremy313/Soteria.git