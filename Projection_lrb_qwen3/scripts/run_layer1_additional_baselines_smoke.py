#!/usr/bin/env python3
"""Run four fixed Qwen3 Layer-1 DAGER baseline smokes from one captured tuple.

This isolated entrypoint evaluates only topk@0.7, topk@0.9, compression@16,
and compression@32.  It is a defense-unaware Layer-1 token-set mechanism
smoke, not a checkpoint, utility, formal-privacy, or adaptive-attack run.
"""
...
from utils.defenses import gradient_compression, topk_sparsification
...
OUTPUT_ROOT = PROJECT_ROOT / "outputs" / "smoke" / "layer1_additional_baselines_v1"
OUTPUT_PATH = OUTPUT_ROOT / "paired_smoke.jsonl"
ALL_SMOKE_OUTPUT_PATH = OUTPUT_ROOT / "paired_smoke_all.jsonl"


class Layer1AdditionalBaselineSmokeError(RuntimeError):
    """Raised when the fixed additional-baseline protocol is violated."""


@dataclass(frozen=True)
class AdditionalBaselineArm:
    defense: str
    preset: str
...
ADDITIONAL_BASELINE_ARMS: tuple[AdditionalBaselineArm, ...] = (
    AdditionalBaselineArm("topk", "topk", "defense_topk_ratio", 0.7, 0.7, None, None, None, "not_applicable"),
    AdditionalBaselineArm("topk", "topk", "defense_topk_ratio", 0.9, 0.9, None, None, None, "not_applicable"),
    AdditionalBaselineArm("compression", "compression", "defense_n_bits", 16, None, 16, None, DEFENSE_RNG_SEED, DEFENSE_SEED_MODE),
    AdditionalBaselineArm("compression", "compression", "defense_n_bits", 32, None, 32, None, DEFENSE_RNG_SEED, DEFENSE_SEED_MODE),
)
...
def _all_registered_smoke_samples(config: Any) -> tuple[Any, ...]:
    """Load exactly the five preregistered smoke samples through the validator."""
    manifest_path = config.project_root / "manifests" / "smoke.jsonl"
    try:
        rows = [json.loads(line) for line in manifest_path.read_text(encoding="utf-8").splitlines()]
    except (OSError, json.JSONDecodeError) as error:
        raise Layer1AdditionalBaselineSmokeError(f"Unable to read preregistered smoke manifest: {error}") from error
    ...
    if len(sample_keys) != 5 or any(not isinstance(key, str) or len(key) != 64 for key in sample_keys):
        raise Layer1AdditionalBaselineSmokeError("The fixed additional baseline smoke requires exactly five valid manifest samples.")
    if len(set(sample_keys)) != len(sample_keys):
        raise Layer1AdditionalBaselineSmokeError("Smoke manifest contains duplicate preregistered sample keys.")
...
def _canonical_gradient_tuple(model: Any) -> tuple[tuple[str, ...], tuple[Any | None, ...], dict[str, Any]]:
    ...
    if names != manifest_names:
        raise Layer1AdditionalBaselineSmokeError("Canonical gradient manifest no longer matches model.named_parameters() order.")
    ...
    if len(gradients) != len(names):
        raise Layer1AdditionalBaselineSmokeError("Canonical gradient tuple construction lost a named parameter position.")
...
def apply_additional_baseline_to_canonical_tuple(
    *, arm: AdditionalBaselineArm, canonical_gradients: tuple[Any | None, ...], canonical_parameter_names: tuple[str, ...]
) -> tuple[Any | None, ...]:
    """Apply one required shared helper to a fresh complete canonical tuple."""
    if len(canonical_gradients) != len(canonical_parameter_names):
        raise Layer1AdditionalBaselineSmokeError("Baseline input must retain one slot for every canonical parameter name.")
    ...
    if arm.defense == "topk":
        ...
        raise Layer1AdditionalBaselineSmokeError(f"Unsupported fixed additional baseline arm {arm.defense!r}.")
    if len(defended) != len(canonical_gradients):
        raise Layer1AdditionalBaselineSmokeError(f"{arm.defense} changed the canonical gradient tuple length.")
    for index, raw_gradient in enumerate(canonical_gradients):
        if raw_gradient is None and defended[index] is not None:
            raise Layer1AdditionalBaselineSmokeError(f"{arm.defense} filled canonical None gradient position {index}.")
    return defended


def _decode_additional_baseline_arms(
    *,
    ...
    decode_arm: Callable[[AdditionalBaselineArm, tuple[Any, Any]], dict[str, Any]],
) -> tuple[tuple[AdditionalBaselineArm, dict[str, Any]], ...]:
    """Transform each arm independently and decode only its observed q_proj pair."""
    decoded: list[tuple[AdditionalBaselineArm, dict[str, Any]]] = []
    for arm in ADDITIONAL_BASELINE_ARMS:
        defended = apply_additional_baseline_to_canonical_tuple(
            arm=arm,
...
def _arm_record(
    *, arm: AdditionalBaselineArm, sample: Any, canonical_q_indices: dict[str, int], core: dict[str, Any], canonical_manifest: dict[str, Any], frozen_tau1: dict[str, Any], config_sha256: str
) -> dict[str, Any]:
    identity = sha256_text("|".join(("qwen3_dager_layer1_additional_baseline_smoke_v1", arm.defense, str(arm.defense_param_value), sample.preregistration_sha256, sample.sample_key, str(SMOKE_HEAD_SEED), SMOKE_DTYPE, str(frozen_tau1["frozen_control_identity_sha256"]), config_sha256)))
    return {
        "schema_version": 1,
        "record_type": "qwen3_dager_layer1_additional_baseline_smoke",
...
def _run_additional_baseline_smoke_samples(args: argparse.Namespace, *, config: Any, samples: tuple[Any, ...], output_path: Path) -> list[dict[str, Any]]:
    ...
    if not samples:
        raise Layer1AdditionalBaselineSmokeError("Additional baseline execution requires at least one preregistered smoke sample.")
    ...
    try:
        ...
        for sample in samples:
            if getattr(bundle.tokenizer, "eos_token_id", None) != sample.eos_token_id:
                raise Layer1AdditionalBaselineSmokeError("Manifest EOS id differs from the loaded Qwen3 tokenizer EOS id.")
            ...
            raw_observations = q_projection_observations_from_canonical_tuple(canonical_gradients=raw_canonical_gradients, canonical_parameter_names=canonical_names, q_parameter_names=captured.q_parameter_names, q_canonical_indices=canonical_q_indices)
            if any(not torch.equal(observed, captured_gradient) for observed, captured_gradient in zip(raw_observations, captured.q_gradients)):
                raise Layer1AdditionalBaselineSmokeError("Raw q_proj observations do not match the one captured complete canonical-gradient tuple.")

            def decode_arm(_arm: AdditionalBaselineArm, observations: tuple[Any, Any]) -> dict[str, Any]:
            ...
            for arm, core in _decode_additional_baseline_arms(canonical_gradients=raw_canonical_gradients, canonical_parameter_names=canonical_names, q_parameter_names=captured.q_parameter_names, canonical_q_indices=canonical_q_indices, decode_arm=decode_arm):
        ...
        raise Layer1AdditionalBaselineSmokeError(str(error)) from error
    finally:
...
def run_additional_baseline_smoke(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    return _run_additional_baseline_smoke_samples(args, config=config, samples=(sample,), output_path=OUTPUT_PATH)


def run_all_additional_baseline_smoke(args: argparse.Namespace) -> list[dict[str, Any]]:
    config = load_experiment_config(CONFIG_PATH, require_dataset_path=False)
    return _run_additional_baseline_smoke_samples(args, config=config, samples=_all_registered_smoke_samples(config), output_path=ALL_SMOKE_OUTPUT_PATH)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run fixed Qwen3 Layer-1 baseline smoke: topk@0.7, topk@0.9, compression@16, and compression@32 only.")
...
def main() -> int:
    args = parse_args()
    try:
        records = run_all_additional_baseline_smoke(args) if args.all_smoke else run_additional_baseline_smoke(args)
    except Exception as error:
        print(json.dumps({"record_type": "qwen3_dager_layer1_additional_baseline_smoke_error", "result_status": "error", "error_type": type(error).__name__, "error": str(error)}, sort_keys=True), file=sys.stderr)
        return 2
