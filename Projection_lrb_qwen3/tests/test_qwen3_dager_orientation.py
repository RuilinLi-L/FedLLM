...
class Qwen3DagerOrientationTest(unittest.TestCase):
    ...
    def test_absolute_rank_matches_root_dager_rule_and_uses_shared_maximum(self) -> None:
        dimension = 32
        ...
        absolute_diagnostic_rank = int(
            torch.linalg.matrix_rank(q0_gradient.float(), atol=0.1, rtol=0.0).item()
        )
        self.assertEqual(absolute_diagnostic_rank, 3)
        ...
        self.assertEqual(selection.rank_definition, "absolute_matrix_rank_atol_rtol_zero")
        self.assertEqual(selection.q0_effective_rank, absolute_diagnostic_rank)
        self.assertEqual(selection.q1_effective_rank, absolute_diagnostic_rank)
        self.assertEqual(selection.requested_shared_rank, absolute_diagnostic_rank)
        self.assertEqual(selection.applied_shared_rank, absolute_diagnostic_rank)
        ...
        self.assertEqual(span.effective_rank, absolute_diagnostic_rank)
        self.assertEqual(span.applied_rank, absolute_diagnostic_rank)
        self.assertEqual(span.absolute_tolerance, 0.1)
