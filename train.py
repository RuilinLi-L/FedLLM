...
class MatthewsCorrelationMetric:
    """CoLA metric without network; matches datasets metric add_batch/compute API."""
    ...
    def add_batch(self, predictions, references):
        ...
        self._predictions.extend(np.asarray(predictions).flatten().tolist())
        self._references.extend(np.asarray(references).flatten().tolist())

    def compute(self):
        out = {
            'matthews_correlation': float(
                matthews_corrcoef(self._references, self._predictions)
            )
        }
        self._predictions = []
        self._references = []
...
def main():
    ...
    if args.dataset == 'cola':
        metric = MatthewsCorrelationMetric()
        train_metric = MatthewsCorrelationMetric()
    else:
