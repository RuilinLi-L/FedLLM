...
OUT_DOCX = OUT_DIR / "FedLLM_LRB_阶段性工作进展与论文整理版_20260505.docx"
OUT_MD = OUT_DIR / "FedLLM_LRB_阶段性工作进展与论文整理版_20260505.md"
...
def set_cell_text(cell, text: str, *, bold=False, size=8.5, color="1f2933", align=None):
    cell.text = ""
    p = cell.paragraphs[0]
...
def set_table_borders(table, color="D4DBE3"):
...
def add_note_box(doc: Document, title: str, lines: list[str], fill="EEF5FB"):
    table = doc.add_table(rows=1, cols=1)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_borders(table, color="B7C6D8")
    cell = table.cell(0, 0)
...
def add_table(doc: Document, headers: list[str], rows: list[list[str]], widths_cm: list[float] | None = None, font_size=8.2):
    ...
    for i, h in enumerate(headers):
        ...
        set_cell_text(cell, h, bold=True, size=font_size, color="17365D", align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    for r_i, row in enumerate(rows):
        ...
        for i, val in enumerate(row):
            ...
            set_cell_margins(cell)
            align = WD_ALIGN_PARAGRAPH.CENTER if len(str(val)) < 28 or i in {0, 2, 3, 4, 5} else WD_ALIGN_PARAGRAPH.LEFT
            set_cell_text(cell, val, size=font_size, align=align)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
            if widths_cm:
                cell.width = Cm(widths_cm[i])
    doc.add_paragraph()
...
def escape_md_cell(text: str) -> str:
    return clean_md_text(text).replace("\\", "\\\\").replace("|", "\\|") or " "
...
def docx_table_to_md(table) -> list[str]:
    rows = [[clean_md_text(cell.text) for cell in row.cells] for row in table.rows]
    if not rows:
        return []
...
def extract_markdown_from_docx(path: Path) -> str:
    doc = Document(str(path))
    lines: list[str] = []
    for kind, block in iter_docx_blocks(doc):
        if kind == "p":
            text = clean_md_text(block.text)
            if not text:
                continue
            ...
            md_lines = docx_table_to_md(block)
            if md_lines:
...
gap_rows = [
    [
        "FL-LLM 中间更新存在文本 recoverability 风险",
        "将防御目标从 generic perturbation 转为 suppress recoverability bottleneck",
        "把问题从是否共享原文推进到共享更新是否携带可恢复文本结构",
        "clean DAGER rec_token_mean=0.833506, R1+R2=141.710856",
    ],
    [
        "topk/compression 在 full-gradient DAGER 下很强，但机制目标偏通信压缩",
        "Projection-LRB 用 layer-wise low-resolution signed projection 建立机制性瓶颈",
        "提供区别于少传/量化的隐私机制叙事，并保留与强 baseline 的正面对比",
        "topk@0.1 和 compression@8 当前 tradeoff 很强；proj_only@0.5 显示新方法空间",
    ],
    [
        "full_lrb 虽能强防御，但 clipping/noise 不是当前主效应且 utility 偏重",
        "通过 ablation 将主方法从 full_lrb 收束为 projection-only / Projection-LRB",
        "避免把过防御配置写成最终方法，使论文主张与消融证据一致",
        "clip_only 几乎不阻断恢复；proj_only@0.5 DAGER=0 且 clean-level utility；full_lrb@0.5 drop=0.020642",
    ],
]
...
source_rows = [
    ["模板", r"D:\code\Projects\FedLLM\docs\初步思路模版.docx"],
    ["Agent 聊天记录", r"D:\code\Projects\FedLLM\docs\聊天记录20260505.md"],
    ["现有成稿", r"D:\code\Projects\FedLLM\output\doc\FedLLM_LRB_阶段性工作进展与实验结果汇总1.docx"],
    ["DAGER baseline 分析", r"D:\code\Projects\FedLLM\docs\DEFENSE_BASELINES_N100_ANALYSIS_20260502.md"],
    ["Utility 分析", r"D:\code\Projects\FedLLM\docs\UTILITY_RESULTS_ANALYSIS_20260426.md"],
    ["LRB 消融分析", r"D:\code\Projects\FedLLM\docs\LRB_ABLATION_ANALYSIS_20260503.md"],
    ["LRB 方法详解", r"D:\code\Projects\FedLLM\docs\LRB_方法详解.md"],
    ["DAGER runs", r"D:\code\Projects\FedLLM\log\runs\defense_baselines_sst2_b2_gpt2_20260501_010024"],
    ["Utility runs", r"D:\code\Projects\FedLLM\log\runs\utility260426"],
    ["Ablation runs", r"D:\code\Projects\FedLLM\log\runs\lrb_ablation_sst2_b2_gpt2_k0.5_20260501_004737"],
]
...
def build_docx():
    ...
    add_table(doc, ["项目", "内容"], meta_rows, widths_cm=[4.0, 12.4], font_size=9.2)
    add_note_box(
        doc,
        "核心结论先行",
        [
            ...
            "仅看 full-gradient DAGER，topk@0.1 与 compression@8 仍是更强经验 tradeoff；full_lrb@0.5 已改善 utility，但仍偏重。",
            "消融显示 projection bottleneck 是主效应：proj_only@0.5 在单次消融运行中达到 DAGER=0 并保持 clean-level utility，因此下一阶段应把主方法收束为 Projection-LRB。",
        ],
    )

    add_heading(doc, "一、文档定位与当前工作状态", 1)
    add_para(doc, "本文将现有阶段性汇总进一步整理成可继续写论文的研究稿。它保留已有实验事实，但把主线从泛称 LRB 调整为 Projection-LRB / LRB-lite，并按照初步思路模板补上关键的 Gap - Novelty - Contribution - Evidence 映射。")
    add_para(doc, "截至 2026-05-05，项目已经从复现 DAGER 与枚举 baseline 推进到更清晰的阶段：full-gradient DAGER 风险已被量化，多个 defense 的 privacy/utility 已经有结果，LRB 的主要组件也通过完整消融被拆开。当前最重要的变化是：主线不再是证明 DAGER 可以被打掉，而是证明哪一种 recoverability bottleneck 能在相近 privacy 下保留更多效用。")
    add_note_box(
        doc,
        "审稿口径下的证据边界",
        [
            "已经完成并可较强支撑：SST2 + GPT2 fine-tuned checkpoint + batch=2 + n_inputs=100 的 full-gradient DAGER attack-time privacy；full training utility fixed points；一次完整 LRB ablation。",
            "尚不能直接外推：LoRA/PEFT 训练期 defense、partial-gradient attack、跨数据集/backbone、多 seed 统计显著性、adaptive attack 与严格 DP accounting。",
    ...
    add_para(doc, "大模型微调正在从集中式数据汇聚转向联邦学习、跨机构协作训练和 PEFT adapter 共享。这样的设置看起来天然适合隐私场景：医疗文本、金融评论、企业客服、用户输入日志等原始文本不离开本地，只上传梯度、模型更新或轻量参数更新。但梯度反演与文本恢复攻击表明，不上传原文并不等价于原文不可恢复。")
    add_para(doc, "本项目当前 clean DAGER 结果给出了直接动机：在 SST2 + GPT2 + batch=2 + n_inputs=100 的设置下，未防御 FedSGD 梯度的 rec_token_mean 达到 0.833506，ROUGE-1+ROUGE-2 达到 141.710856。这说明攻击者并不是只能恢复模糊主题，而是能够恢复大量 token 级文本信息。")
    add_para(doc, "进一步的动机来自 privacy-utility 冲突：简单把 DAGER 打到 0 并不困难，困难在于保留训练效用。当前实验显示，完整 LRB 能稳定抑制 DAGER，但 utility 代价偏高；topk/compression 在 full-gradient DAGER 下很强，却更像通信压缩副作用而不是针对文本 recoverability 的机制解释。")

    add_heading(doc, "三、前人工作局限性与创新位置", 1)
    add_para(doc, "已有工作为本项目提供了攻击、隐私和压缩三类重要参照，但各自都留下了适合本工作切入的缺口。当前实验也印证了这些局限：强扰动方法通常牺牲 utility，通信压缩方法在当前 DAGER 下表现强但机制目标不完全对齐，表示扰动和训练侧混合方法在 GPT2/FedSGD 设置中并不稳定。")
    add_table(doc, ["方向", "代表思路", "当前局限", "对本工作的启发"], related_rows, widths_cm=[3.0, 4.1, 4.9, 4.6], font_size=8.0)
    add_heading(doc, "四、关键映射表：Gap - Novelty - Contribution - Evidence", 1)
    add_para(doc, "这是模板中最关键、此前缺失的映射表。它把研究瓶颈、创新动作、贡献证据和当前实验事实对应起来，便于后续直接转成 Introduction 的 contribution 段或论文主表叙事。")
    add_table(doc, ["Gap / 瓶颈", "Novelty / 创新动作", "Contribution / 贡献", "Evidence / 已有证据"], gap_rows, widths_cm=[4.0, 4.3, 4.3, 4.1], font_size=7.8)

    add_heading(doc, "五、希望解决的问题与应用意义", 1)
    add_para(doc, "本工作希望解决的不是单一实验点上的攻击是否成功问题，而是 FL-LLM 中间更新如何在可训练与不可恢复之间取得平衡的问题。研究问题应拆成风险成立、防御有效、效用可接受、机制清楚和场景泛化五个层次。")
    add_table(doc, ["问题层次", "要回答的问题", "当前已有依据", "下一步补强"], problem_rows, widths_cm=[2.8, 4.7, 4.7, 4.2], font_size=7.9)
    add_para(doc, "该问题对应的真实场景是多方拥有敏感文本、但又需要共同微调或适配语言模型的协作训练，例如医院之间联合训练临床文本分类模型、金融机构联合优化舆情或风控文本模型、企业之间共享客服/工单语义能力，以及移动端或边缘端更新个性化语言模型。")

    add_heading(doc, "六、威胁模型与评价对象", 1)
    add_para(doc, "本工作关注大模型联邦训练或分布式微调过程中由中间更新信息导致的数据泄露。客户端不上传原始文本，但会上传梯度、模型更新或 PEFT adapter 更新；攻击者可能据此恢复训练样本。关键不在于数据有没有直接共享，而在于共享更新本身是否携带足以恢复原始文本的高分辨率信息。")
    add_table(doc, ["威胁模型要素", "当前实验默认假设", "审稿风险 / 后续补充"], threat_rows, widths_cm=[3.2, 6.6, 6.4], font_size=8.0)

    add_heading(doc, "七、Projection-LRB 方法机制", 1)
    add_para(doc, "根据当前消融证据，文档中的主方法应从泛称 LRB 收束为 Projection-LRB：一个面向 recoverability 的 layer-wise gradient projection defense。full_lrb 仍作为强防御 / 过防御对照保留，用来说明 clipping 与 residual-space noise 并不是当前 full-gradient DAGER 设置下的必要主效应。")
    add_para(doc, "更适合论文的方法表述是：对每层梯度 g_l，先根据结构先验与当前 batch 梯度统计估计敏感度 s_l，再用 keep ratio k_l 控制低分辨率 signed_pool 投影 P_{k_l}(g_l)。Projection-LRB 的核心更新可写作 \\tilde{g}_l=P_{k_l}(g_l)，其中高敏感层使用更低分辨率 bottleneck；full_lrb 则在此基础上额外加入 clipping 与 residual-space noise。")
    add_table(doc, ["模块", "当前实现", "核心作用", "消融关注点"], method_rows, widths_cm=[3.5, 4.8, 4.8, 3.4], font_size=8.0)
    add_note_box(
        doc,
        "推荐当前方法表述",
        [
    ...
    add_heading(doc, "八、实验设置与指标", 1)
    add_table(doc, ["实验块", "日志目录 / 文档", "关键设置", "用途"], setup_rows, widths_cm=[3.0, 5.0, 4.7, 3.8], font_size=7.6)
    add_table(doc, ["指标", "含义", "解读注意"], metric_rows, widths_cm=[3.2, 7.2, 5.8], font_size=8.0)

    add_heading(doc, "九、DAGER Defense Baselines：privacy 结果", 1)
    add_para(doc, "正式 n_inputs=100 结果强化了一个判断：在 SST2+GPT2+batch=2 的 full-gradient DAGER 设置下，clean FedSGD 泄露非常严重。真正进入 strong privacy baseline 的主要是 LRB、Top-k 和 Compression 三类，但 DAGER=0 只表示当前攻击未恢复，不等价于严格隐私保证。")
    add_table(doc, ["defense", "参数区间 / 关键点", "rec_token_mean", "R1+R2", "结论"], privacy_rows, widths_cm=[2.4, 4.2, 3.0, 2.7, 4.3], font_size=7.8)

    add_heading(doc, "十、End-to-End Utility 与 Pareto 判断", 1)
    add_para(doc, "Utility 结果说明，能够把 DAGER 打到 0 并不自动意味着 defense 好。当前 fixed-point 下，topk@0.1 与 compression@8 在保持 DAGER=0 的同时几乎不损失 accuracy；full_lrb@0.5 明显改善了 utility，却仍弱于 topk/compression 的强经验点。")
    add_table(doc, ["defense", "param", "accuracy", "macro-F1", "loss", "utility_drop", "train_time"], utility_rows, widths_cm=[2.2, 1.6, 2.2, 2.2, 2.1, 2.2, 2.3], font_size=7.8)
    add_table(doc, ["层级", "方法", "事实依据", "当前判断"], pareto_rows, widths_cm=[2.4, 4.0, 5.0, 5.0], font_size=7.8)

    add_heading(doc, "十一、LRB 消融：projection 是主效应", 1)
    add_para(doc, "完整消融结果是当前最有价值的新证据。它显示 full_lrb 并不是当前最优形式：真正的主效应来自低分辨率 signed_pool projection。proj_only@0.5 在不加 residual noise、不做额外 clipping 的情况下已经把 DAGER rec_token_mean 和 ROUGE 恢复全部降到 0，并在该次运行中保持 clean-level accuracy。")
    add_table(doc, ["variant", "rec_token", "R1+R2", "accuracy", "drop", "train_time", "attack_time", "机制结论"], ablation_rows, widths_cm=[2.8, 1.8, 1.8, 1.8, 1.7, 2.1, 2.1, 3.8], font_size=7.2)
    add_note_box(
        doc,
        "消融给出的关键改口",
        [
    ...
    add_heading(doc, "十二、AAAI / 审稿口径下的 Claims 与 Non-Claims", 1)
    add_para(doc, "这部分用于控制论文叙事边界。审稿人最敏感的是 claims 与 evidence 是否匹配，因此要正面承认当前证据边界，而不是把 DAGER=0 或单次消融结果写成通用安全结论。")
    add_table(doc, ["类型", "建议表述"], limitations_rows, widths_cm=[3.0, 13.2], font_size=8.2)

    add_heading(doc, "十三、主文图表最小集合", 1)
    add_para(doc, "根据 agent 建议，后续论文主文不应堆满所有 baseline sweep，而应保留能闭环主论点的最小图表集合：风险锚点、威胁模型、方法框架、主结果、消融，以及一个 transfer/runtime 预留位。")
    add_table(doc, ["编号", "图表名称", "论文位置", "当前状态 / 用途"], figure_table_rows, widths_cm=[2.0, 4.8, 3.3, 6.4], font_size=7.9)

    add_heading(doc, "十四、下一步优先级", 1)
    add_table(doc, ["优先级", "任务", "目标", "建议输出"], priority_rows, widths_cm=[1.7, 4.8, 5.0, 5.0], font_size=7.9)

    add_heading(doc, "十五、可以直接用于汇报的阶段性结论", 1)
    add_para(doc, "当前 SST2+GPT2+batch=2 的 full-gradient DAGER 实验表明，clean FedSGD 梯度存在严重文本恢复风险。full_lrb 完整配置能稳定把 DAGER 恢复压到 0，但其 clipping 和 residual noise 带来了明显 utility 代价。消融进一步显示，低分辨率 signed_pool projection 是当前 LRB 有效性的主因：proj_only@0.5 在不加噪、不做完整裁剪的情况下即可实现 DAGER=0，并在单次运行中保持 clean-level accuracy。因此，下一阶段应把主方法身份统一为 Projection-LRB / LRB-lite，并通过 keep-ratio sweep、多 seed、LoRA/PEFT、partial-gradient 和跨数据集实验验证其泛化价值。")
    ...
    add_heading(doc, "十六、材料来源清单", 1)
    add_table(doc, ["类别", "路径"], source_rows, widths_cm=[4.0, 12.4], font_size=8.0)
    doc.save(OUT_DOCX)
...
def build_markdown() -> str:
    ...
    h(1, "FedLLM / Projection-LRB 阶段性工作进展与论文整理稿")
    p("基于现有实验、初步思路模板与 agent 建议整理；不包含 Overleaf / LaTeX 生成。")
    table(["项目", "内容"], meta_rows)
    h(2, "核心结论先行")
    for line in [
        "当前工作已经形成 DAGER 攻击、防御 baseline、utility、LRB 机制消融的闭环证据，但强证据边界主要是 SST2+GPT2+batch=2 的 full-gradient DAGER。",
        "Clean FedSGD 泄露严重；LRB、topk、compression 均可在若干点上把当前 DAGER 恢复打到 0，但这不等价于形式化隐私保证。",
        ...
        p(line)
    h(2, "一、文档定位与当前工作状态")
    p("本文将现有阶段性汇总进一步整理成可继续写论文的研究稿。它保留已有实验事实，但把主线从泛称 LRB 调整为 Projection-LRB / LRB-lite，并按照初步思路模板补上关键的 Gap - Novelty - Contribution - Evidence 映射。")
    p("截至 2026-05-05，项目已经从复现 DAGER 与枚举 baseline 推进到更清晰的阶段：full-gradient DAGER 风险已被量化，多个 defense 的 privacy/utility 已经有结果，LRB 的主要组件也通过完整消融被拆开。当前最重要的变化是：主线不再是证明 DAGER 可以被打掉，而是证明哪一种 recoverability bottleneck 能在相近 privacy 下保留更多效用。")
    h(2, "二、背景与动机")
    p("大模型微调正在从集中式数据汇聚转向联邦学习、跨机构协作训练和 PEFT adapter 共享。这样的设置看起来天然适合隐私场景：医疗文本、金融评论、企业客服、用户输入日志等原始文本不离开本地，只上传梯度、模型更新或轻量参数更新。但梯度反演与文本恢复攻击表明，不上传原文并不等价于原文不可恢复。")
    p("本项目当前 clean DAGER 结果给出了直接动机：在 SST2 + GPT2 + batch=2 + n_inputs=100 的设置下，未防御 FedSGD 梯度的 rec_token_mean 达到 0.833506，ROUGE-1+ROUGE-2 达到 141.710856。")
    h(2, "三、前人工作局限性与创新位置")
    table(["方向", "代表思路", "当前局限", "对本工作的启发"], related_rows)
    h(2, "四、关键映射表：Gap - Novelty - Contribution - Evidence")
    table(["Gap / 瓶颈", "Novelty / 创新动作", "Contribution / 贡献", "Evidence / 已有证据"], gap_rows)
    h(2, "五、希望解决的问题与应用意义")
    table(["问题层次", "要回答的问题", "当前已有依据", "下一步补强"], problem_rows)
    h(2, "六、威胁模型与评价对象")
    table(["威胁模型要素", "当前实验默认假设", "审稿风险 / 后续补充"], threat_rows)
    h(2, "七、Projection-LRB 方法机制")
    p("根据当前消融证据，文档中的主方法应从泛称 LRB 收束为 Projection-LRB：一个面向 recoverability 的 layer-wise gradient projection defense。full_lrb 仍作为强防御 / 过防御对照保留。")
    p("对每层梯度 g_l，先根据结构先验与当前 batch 梯度统计估计敏感度 s_l，再用 keep ratio k_l 控制低分辨率 signed_pool 投影 P_{k_l}(g_l)。Projection-LRB 的核心更新可写作 \\tilde{g}_l=P_{k_l}(g_l)。")
    table(["模块", "当前实现", "核心作用", "消融关注点"], method_rows)
    h(2, "八、实验设置与指标")
    table(["实验块", "日志目录 / 文档", "关键设置", "用途"], setup_rows)
    table(["指标", "含义", "解读注意"], metric_rows)
    h(2, "九、DAGER Defense Baselines：privacy 结果")
    table(["defense", "参数区间 / 关键点", "rec_token_mean", "R1+R2", "结论"], privacy_rows)
    h(2, "十、End-to-End Utility 与 Pareto 判断")
    table(["defense", "param", "accuracy", "macro-F1", "loss", "utility_drop", "train_time"], utility_rows)
    table(["层级", "方法", "事实依据", "当前判断"], pareto_rows)
    h(2, "十一、LRB 消融：projection 是主效应")
    table(["variant", "rec_token", "R1+R2", "accuracy", "drop", "train_time", "attack_time", "机制结论"], ablation_rows)
    h(2, "十二、AAAI / 审稿口径下的 Claims 与 Non-Claims")
    table(["类型", "建议表述"], limitations_rows)
    h(2, "十三、主文图表最小集合")
    table(["编号", "图表名称", "论文位置", "当前状态 / 用途"], figure_table_rows)
    h(2, "十四、下一步优先级")
    table(["优先级", "任务", "目标", "建议输出"], priority_rows)
    h(2, "十五、可以直接用于汇报的阶段性结论")
    p("当前 SST2+GPT2+batch=2 的 full-gradient DAGER 实验表明，clean FedSGD 梯度存在严重文本恢复风险。full_lrb 完整配置能稳定把 DAGER 恢复压到 0，但其 clipping 和 residual noise 带来了明显 utility 代价。消融进一步显示，低分辨率 signed_pool projection 是当前 LRB 有效性的主因：proj_only@0.5 在不加噪、不做完整裁剪的情况下即可实现 DAGER=0，并在单次运行中保持 clean-level accuracy。因此，下一阶段应把主方法身份统一为 Projection-LRB / LRB-lite。")
    p("In the current SST2/GPT2 full-gradient DAGER setting, the dominant effective component is the low-resolution signed projection bottleneck. While the full LRB configuration suppresses token recovery to zero, its clipping and residual-space noise introduce unnecessary utility loss.")
    h(2, "十六、材料来源清单")
    table(["类别", "路径"], source_rows)
    return "\n".join(parts).strip() + "\n"
...
def main():
    ...
    build_docx()
    OUT_MD.write_text(extract_markdown_from_docx(OUT_DOCX), encoding="utf-8")
    print(OUT_DOCX)
    print(OUT_MD)
